...
> ⚠️ **NOTE: Generics Work Only with Reference Types**
>
...
>
> ```java
> ArrayList<int[]> a = new ArrayList<>();  // ✅ Allowed
> ```
...
- Generics make errors appear at compile time rather than at run time (it's always better to know problems in your code at compile time rather than making your code fail at run time).
...
## ⚖️ Generics vs Wildcards

| Feature | Generics | Wildcards |
|---|---|---|
| **Definition** | Parameterize the type of a class/method. | Represent an unknown type. |
| **Flexibility** | Fixed at declaration time. | Flexible for method parameters. |
| **Use Case** | Strongly typed, reusable code. | Allow varying types in collections. |
